...
enc = Encoder(INPUT_DIM, ENC_EMB_DIM, ENC_HID_DIM, DEC_HID_DIM, LAYERS, ATTN_ENC_DROPOUT, SRC_PAD_IDX, device, QnA_vocab.vectors)
    
print("==> Building Decoder")
    
attn = Attention(ENC_HID_DIM, DEC_HID_DIM, device)
dec = Decoder(OUTPUT_DIM, DEC_EMB_DIM, ENC_HID_DIM, DEC_HID_DIM, ATTN_DEC_DROPOUT, TRG_PAD_IDX, device, QnA_vocab.vectors, attn)
...
#train_iter, val_iter, test_iter = BucketIterator((train_data, val_data, test_data), batch_size = BATCH_SIZE, sort_key=lambda x: len(x.Q)+len(x.A), sort_within_batch = True, device = device, repeat= False)
    for fold, (train_dataset, val_dataset) in enumerate(train_val_generator):
        ...
        train_loss = attn_train(model, train_iter, optimizer, criterion, CLIP)
            
        print("\t\tTrain Loss: {:.3f} | Train PPL: {:7.3f}".format(train_loss,math.exp(train_loss)))
        
        valid_loss = attn_eval(model, val_iter, criterion)        
            
        if valid_loss < best_valid_loss:
            best_valid_loss = valid_loss
            torch.save(model.state_dict(), 'checkpoints/attn-model.pt')
...
test_loss = attn_eval(model, test_iter, criterion)
    
print("| Test Loss: {:.3f} | Test PPL: {:7.3f} |".format(test_loss,math.exp(test_loss)))

print("==> Start Evaluation...")
    
bleu_score, precision = calculate_bleu("ATTN",test_data, Q, A, Ans_Sen, QnA_vocab, model, device)
